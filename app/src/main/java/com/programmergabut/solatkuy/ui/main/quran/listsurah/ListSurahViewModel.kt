...
class ListSurahViewModel @ViewModelInject constructor(private val quranRepository: QuranRepository): ViewModel() {
    ...
    val allSurah: LiveData<Resource<List<Data>?>> = _allSurah
    fun fetchAllSurah(){
            ...
            try{
                ...
                if(response.statusResponse == "1"){
                    val mappedResponse = response.data.map { surah ->
                        val searchAbleName = surah.englishName.toLowerCase(Locale.getDefault()).replace("-", " ")
                        Data(
                            surah.englishName, searchAbleName, surah.englishNameTranslation, surah.name,
                            surah.number, surah.numberOfAyahs,surah.revelationType
                        )
                    }
